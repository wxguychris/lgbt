<?php
/**
 *  this page processes the submitted information from the donation form and 
 *  redirects the user to the payment processor's site
 * 
 */
...
if (isset($_POST['Products'])) {
    //product was submitted -- generate information and hand off to processor
    if ($_POST['Products']=='recurring') {
        //this is a recurring payment
        $recur_amt=preg_replace("/[^0-9\.]/", "", $_POST['Amount']);
        if ($recur_amt<=0) {
            //no value passed in the recurring field, bounce back to error page
            header("Location: {$payment['xxxCancelURL']}");
        } else {
            //positive, non-zero value passed
            //add recurring item to the products list
            $payment['Products'] .= "|$recur_amt::1::recur::Recurring Donation (Monthly)::{RB amount=$recur_amt startmonth=+1 frequency=monthly duration=0 email=2}";
        }
    } else {
        //this is a 1-time/1-item donation
        $payment['Products'] = $_POST['Products'];
    }      
} else {
    //no product was submitted -- there's an error somewhere.
    header("Location: {$payment['xxxCancelURL']}");
}
...
<title>LGBT Center of Raleigh - Contribute</title>
<style type="text/css">
<!--
body {
...
<body onLoad="MM_preloadImages('../images/join_email_list_over.gif','../images/facebook_over.jpg');javascript:document.forms['form'].submit();"><table border="0" cellspacing="0" cellpadding="0">
              <tr>
...
<!-- #EndLibraryItem --></td>
            ...
            <td width="599"><table border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                ...
                <td><p class="bluebold">Please wait to complete your donation.</p>
                  <p class="body">
                    ...
                    ?>
                        ...
                        <br/><br/><input type="submit" value="Submit" class="bodysmall"/>
                    </form>
				  </p></td>
